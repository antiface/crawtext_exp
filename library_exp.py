...
def make_seeds(query,path,nb_results=1,expansion=1,lang="fr"):
	...
	seeks_node = 'seeks.fr' #'67.23.28.136'
	query_seeks = '%s' % urllib.urlopen('http://%s/search.php/search/q=%s?lang=%s&output=json&rpp=%d&expansion=%d' % ( seeks_node, query.replace(' ','+'), lang, nb_results, expansion )).read()
